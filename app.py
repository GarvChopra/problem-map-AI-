...
def issue_detail(issue_id):
    ...
    try:
        ...
        # Each step is "done" once that milestone is true. Verified is a flag
        # (is_verified), escalated is a flag (is_escalated), resolved is the
        # terminal status. The "active" step is the next-to-do one — i.e. the
        # leftmost pending step.
        is_verified  = bool(issue.get('is_verified'))
        is_escalated = bool(issue.get('is_escalated'))
        ...
        order = ['Open', 'Verified', 'Escalated', 'Resolved']
        timeline = []
        active_assigned = False
        for label in order:
            if step_done[label]:
                timeline.append({'label': label, 'state': 'done'})
            elif not active_assigned:
                timeline.append({'label': label, 'state': 'active'})
                active_assigned = True
            else:
                timeline.append({'label': label, 'state': 'pending'})
